 ...
 *  de adornos navideños. El primer adorno que quieren fabricar
...
Como ves el cubo tiene tres caras visualmente.
 Los símbolos que se usan para construir las caras 
 del cubo son: /, \, _ y (espacio en blanco).
...
Cada nueva línea del cubo debe terminar con un salto 
de línea (\n) excepto la última.
 */
  ...
  for (let index = 0; index < size; index++) {
    ...
    if (index == size - 1) {
      for (let y = 0; y < size; y++) {
        //   console.log(size - y, "/\\".repeat(size - y));
        let horizontalLine =
          " ".repeat(y + 1) +
          "\\".repeat(1) +
          "/\\".repeat(size - y) +
