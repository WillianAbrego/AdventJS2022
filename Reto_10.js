 ...
 * hace una parabola al saltar entre ciudades. Recibes un array 
 * de números que representan la altura en la que se encuentra 
 * el trineo en cada momento.

Para que la parabola sea correcta, el viaje del trineo debe ser 
ascendente al principio, llegar al punto más alto y 
descender hasta el final. No puede volver a subir una vez que ha bajado,
...
function checkJump(heights) {
  ...
  for (let x = 1; x < heights.length; x++) {
    //console.log(heights[x - 1], heights[x]);
    if (heights[x - 1] < heights[x] && acum != false) {
      acum = true;
    } else if (heights[x - 1] > heights[x]) {
      acum = false;
    } else {
      otraSubida++;
    }
