...
Tiene un array de ciudades donde cada elemento es el número de
 regalos que puede dejar allí. [12, 3, 11, 5, 7]. Por otro lado,
  el límite de regalos que caben en su saco. Y, finalmente,
   el número de ciudades máximo que sus renos pueden visitar.
...
Si no se puede realizar ningún viaje que satisface los
 requerimientos, el resultado debe ser 0. Más ejemplos:
...
function getMaxGifts(arr, num, maxCities) {
  let maxciti = 0;

  for (m = 1; m < Math.pow(2, arr.length); m++) {
    let comb = [];

    for (i = 0; i < arr.length; i++) if (Math.pow(2, i) & m) comb.push(arr[i]);
    let sum = 0;
    for (i = 0; i < comb.length; i++) sum += comb[i];

    if (sum === num && comb.length >= maxCities) {
      //console.log("Eureka!", comb, sum, num);
      maxciti = sum;
      // console.log("max!", comb, sum, num);
